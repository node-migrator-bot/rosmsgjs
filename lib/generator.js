...
//    `createFromPackage`.
//  * options     - object with list of templating options. Options include:
//    * rosnodejsDir - Path to use for requiring the rosnodejs module. Defaults
//      to 'rosnodejs'.
messageGenerator.template = function(packageName, messages, options, callback) {
...
messageGenerator.createFromPackage = function(packageName, callback) {
  ...
  getAllMessageTypes(packageName, function(error, messageTypes) {
    if (error) {
      ...
      messageGenerator.createFromMessageTypes(messageTypes, callback)
    }
  })
}
