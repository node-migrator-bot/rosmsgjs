ros.Message.extend({
  ...
  , initialize: function(attributes) {<% for (var i = 0; i < fields.length; i++) { var fieldName = fields[i]; var fieldMessage = fieldMessages[fieldName]; var fieldDefault = fieldDefaults[fieldName]; if (fieldMessage && !(fieldDefault instanceof Array)) { %>
  ...
  , getMessageForField: function(fieldName) {
      var Message = null<% for (var i = 0; i < fields.length; i++) { var fieldName = fields[i]; var templatedFieldMessage = templatedFieldMessages[fieldName]; if (templatedFieldMessage) { %>
